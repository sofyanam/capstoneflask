...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(['Category','App']).agg({'Reviews':'mean','Rating':'mean'}).sort_values(by='Reviews', ascending=False).reset_index().head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    pscontent = pd.crosstab(index=playstore['Content Rating'],columns='Jumlah').sort_values(by='Jumlah', ascending=False).reset_index().head(3)
    X = pscontent['Content Rating'].values
    Y = pscontent['Jumlah'].values
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    plt.bar(X,Y)
